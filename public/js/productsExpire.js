            ...
            try {
                ...
                div.innerHTML = `
                    ...
                    <p class="sku">SKU: ${producto.SKU}</p>
                       <p class="precio">
                        C$ ${precioAjustado}
                        ${cambioRealizado ? '<i style="font-size:7px;" class="bi bi-asterisk"></i>' : ''}
                    </p>
                `;
...
async function previsualizarPDF() {
    ...
    const nombreArchivo = `${categoriaLimpia}_${fechaActual}_${tipoCliente}.pdf`;

    // Agregar la portada con la categoría y subcategoría
    agregarPortada(pdf, categoriaSeleccionada, subcategoriaSeleccionada, tipoCliente);
    ...
    for (const producto of productosMostrados) {
        ...
        switch (tipoCliente) {
            case 'minorista':
                // Calcular el precio minorista o supermercado con un 15% adicional
                const precioMinorista = Math.round((precioBase * 0.15) + precioBase);
                precioAjustado = precioMinorista;
                break;
            case 'xdocena':
        ...
        pdf.text(`C$ ${precioAjustado}`, x, y + altoImagen + 20, { align: 'left' });
...
function agregarPortada(pdf, categoriaSeleccionada, subcategoriaSeleccionada, tipoCliente) {
    ...
    pdf.setTextColor(...colorTitulo); // Negro para el texto principal
